...
port number). 127.0.0.1 , port number:8000
2. Is it UDP or TCP? How do you know?
TCP, since SOCK_STREAM means TCP
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? the client read data from user
input(STDIN), you can tell from read(STDIN_FILENO,...)
4. How does the client program end? How can you tell that in the code?
when read() returns <=1, meaning empty input or error
*/
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
...
void *handle_client(void *arg) {
  ...
  while ((numread = read(cfd, buf, BUF_SIZE)) > 0) {
    ...
    total_message_count++;
    int count = total_message_count;
    pthread_mutex_unlock(&count_mutex);

    printf("MSG #  %d; Client ID %d: %s", count, client_id, buf);
  }

  if (numread <= 0) {
    printf("Ending thread for cleint %d\n", client_id);
    close(cfd);
  }
