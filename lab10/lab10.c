...
client use TCP(SOCK_STREAM) on IPv4(AF_INET) on port 8001, using write() to send
data to server
2. What data is the client sending to the server?
sending 5 messages "Hello""Apple""Car""Green""Dog"
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
run is to control when the thread should stop accepting
list_handle point to the shared linked list
list_lock is mutex to protect the shared list
2. How are received messages stored?
they are stored in a singly-linked list with a dummy head node
3. What does `main()` do with the received messages?
it calls collect_all() to traverse the linked list and print the messages
4. How are threads used in this sample code?
one acceptor thread to accept client connection, client threads(up to 4) handle
each client.
*/
/*
* How are sockets made non-blocking?
  - Sockets are made non-blocking using fcntl() with the O_NONBLOCK flag
* What sockets are made non-blocking?
  - The server socket (sfd) in run_acceptor()
  - All client sockets (cfd) in run_client()
  - Every socket used for I/O in this program is non-blocking
* Why are these sockets made non-blocking? What purpose does it serve?
  - Non-blocking allows threads to poll for activity without getting stuck
  - For the server socket: accept() returns immediately with EAGAIN if no client
    is waiting, allowing the acceptor thread to check its run flag periodically
  - For client sockets: read() returns immediately with EAGAIN if no data is
    available, allowing client threads to check their run flag periodically
*/
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
