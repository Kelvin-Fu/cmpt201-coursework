...
// Algorithm COALESCE(new_free_block):
//
//  prev_block = block immediately before new_free_block
//   next_block = block immediately after new_free_block
//
//  if prev_block is free:
//      merge prev_block and new_free_block
//      new_free_block = prev_block

// if next_block is free:
//     merge new_free_block and next_block

// Update the free list links so that the merged block replaces the old ones
